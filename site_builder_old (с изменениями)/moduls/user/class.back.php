...
class B_articles_ {
 ...
 function addSub(&$_this,&$sub,&$r,$param) {
  ...
  function &getParamMngr(&$_this) {
         ...
         //город
         if ($_GET['pre_master'] > 0) $where .= ' and u.pre_master= '.$_GET['pre_master'];
         ...
         if ($_GET['id_city'] > 0)
            $param['query'] = "select u.id as id,  u.* from city c,users u where not( u.is_master=0 and u.pre_master=0) c.id=u.id_city  $where $order";
         else
            $param['query'] = "select u.* from users u where not( u.is_master=0 and u.pre_master=0) $where $order";
