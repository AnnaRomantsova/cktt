...
//include($inc_path.'/service/func.service.php');
  ...
  function addSprav(&$main,$table_name,$selected_id,$sub_name,$order='') {
         global $db;
         if ($order == '') $order=$name;
         $r1 = new Select ( $db, "select * from $table_name order by $order" );
         while ($r1->next_row() > 0) {
                   unset($sub);
                   $sub = new outTree();
 ...
 //список разделов мастера
  ...
  //разделы юзера
  function add_user_types(&$main,$user_id,$text_limit = false) {
         global $db;
         $r1 = new Select($db,"select t.id,t.name,t.name_list,t.parent from user_types zt,types t
                              where zt.id_type = t.id and id_user=$user_id");
         $i=1;
         while ($r1->next_row()) {
            unset($sub);
            $sub = new outTree();
            ...
            $sub->addField('name',$r1->result('name_list'));
            if ($i < $r1->num_rows()) $sub->addField('zpt',', ');
...
//разделы юзера верхний уровень
  function add_user_top_types(&$main,$user_id,$text_limit = false) {
         global $db;
         $r1 = new Select($db,"select distinct ts.id, ts.name from user_types zt,types t,types_sections ts
                                  where t.parent=ts.id and zt.id_type = t.id and id_user=$user_id");
         $i=1;
         while ($r1->next_row()) {
            unset($sub);
            $sub = new outTree();
  ...
  function add_user_avatar(&$main,$r) {
        ...
        if (isset($main->not_image1)){
                $tmp = new outTree();
                $tmp->addField('src', '/i/no-photo.png' );
